...
class RR:
    ...
    def fit(self, degree, l, X, y):
        self.degree = degree
        self.l = l
        f = PolynomialFeatures(self.degree).fit_transform(X).transpose()
        I = np.eye(N=f.shape[0])
        inv = np.linalg.inv(np.dot(f, f.transpose()) + I * self.l)
...
def deg_draw(X, y, degrees, lam, split_value=0.2, save='result_ridge_regression_{}.png'):
    ...
    plt.scatter(X, y)
    for l in lam:
        rr.fit(X=X_train, y=y_train, degree=degrees, l=l)
        pred = rr.predict(X_test)
        print("Dergee = {} , Lam = {},  Score(R2) = {}".format(degrees, l, r2_score(y_test, pred)))
        draw(degree=degrees, weight=rr.weight)
        legend.append("Lam = {}".format(l))
...
def main():
    ...
    deg_draw(X, y, degrees=degree, lam=[alpha], split_value=split_value)
