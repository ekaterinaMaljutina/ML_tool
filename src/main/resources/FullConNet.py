...
def main():
    ...
    if value.lr is None and value.func_act is not None:
        ...
        fit_and_predict(full_connent_network(activaction_=func_activation), data_dict,
                        "_".join(["full_connect_{}".format(func_activation), ""]))
    elif value.lr is None and value.func_act is not None:
        lr = float(value.lr)
        func_activation = value.func_act
        fit_and_predict(full_connent_network(activaction_=func_activation, lr=lr), data_dict,
                        "_".join(["full_connect_{}".format(func_activation), ""]))
    else:
        fit_and_predict(full_connent_network(), data_dict,
                        "_".join(["full_connect_{}".format('tanh'), ""]))
